...
def mis_motos_view(request):
    ...
    lista = Historial.objects.filter(propietario = propietario)
...
def moto_editar_view(request, id_moto):
    moto = Historial.objects.get(id=id_moto)
    if request.method == 'GET':
        formulario = agregar_moto_form(instance=moto)
    else:
        formulario = agregar_moto_form(request.POST, instance=moto)
        if formulario.is_valid():
            formulario.save()
            return redirect('/mis_motos/')    
...
def moto_eliminar_view(request, id_moto):
    ...
    return render(request, 'motos/moto_eliminar.html', locals())
...
def mantenimientos_view(request):
    lista = Mantenimientos.objects.filter()
    return render(request, 'motos/mantenimientos.html', locals())

def mantenimiento_agregar_view(request, id_moto, id_historial):
    moto = Historial.objects.get(moto = id_moto)
    historial = Historial.objects.get(id = id_historial)
    if request.method == 'POST':
        formulario = mantenimiento_agregar_form(request.POST, request.FILES)
        if formulario.is_valid():  
            h = formulario.save(commit=False)
            h.id_historial = historial 
            h.save()
...
def mantenimiento_editar_view(request, id_mantenimiento):
    ...
    if request.method == 'GET':
        ...
        formulario = agregar_moto_form(request.POST, instance=mantenimiento)
        if formulario.is_valid():
            formulario.save()
            return redirect('/mis_motos/')    
    return render(request, 'motos/mantenimiento_editar.html', locals())

def mantenimiento_eliminar_view(request):
    return render(request, 'motos/mantenimiento_eliminar.html', locals())    
...
def traspasos_view(request):
    lista = Historial.objects.filter(estado = 'Pendiente', propietario__id_usuario = request.user)
...
def traspaso_aceptar_view(request, id_historial):
    ...
    historial.save()
    propietario_anterior = Historial.objects.get(moto = historial.moto, estado = 'Pendiente')
